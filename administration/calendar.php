...
var actual = an + "-" + mes + "-" +  dia;




$.post("includes/calendario.php", function(data) { 
  
var x = JSON.parse(data);

//console.log(x);
for(var i = 0; i <= x.length - 1; i++){
    x[i]["title"] = x[i]["nombre_servicio"];
}
...
headerToolbar: {
  left: 'title',
  center :'dayGridMonth,dayGrid,dayGridWeek',
...
eventDidMount : function(info) {
    var titulo = info.el.getElementsByClassName("fc-event-title")[0];
        //console.log(info.event.extendedProps["description"]);
        titulo.innerHTML = info.event.extendedProps["nombre_cliente"] + " " +  info.event.extendedProps["apellido_cliente"] ;
}
...
<script>
/*
let hora_completa = document.querySelector("#calendar > div.fc-view-harness.fc-view-harness-passive > div > table > tbody > tr > td > div > div > div > table > tbody > tr > td > div > div.fc-daygrid-day-events > div:nth-child(4) > a > div.fc-event-title").textContent;

    let texto = hora_completa.innerHTML;

    alert("don't worry, it's a test by vista web, we are working on the website");
